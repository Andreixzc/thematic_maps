...
class WaterQualityPredictor:
    ...
    def get_prediction_stats(self, predicted_image, aoi):
        """Calculate min and max values for the predicted parameter"""
        stats = predicted_image.reduceRegion(
        ...
        # The stats will be returned as {band_name_min: min_value, band_name_max: max_value}
...
def process_reservoirs(reservoirs, date_range, output_base_dir='predictions'):
    ...
    for reservoir_name, geometry in reservoirs.items():
        print(f"\nProcessing reservoir: {reservoir_name}")
        ...
        for parameter_dir in reservoir_model_dir.iterdir():
            if parameter_dir.is_dir():
                ...
                try:
                    ...
                    # Get and print min/max values
                    min_value, max_value = predictor.get_prediction_stats(predicted_image, geometry)
                    print(f"\nPrediction statistics for {parameter_name}:")
