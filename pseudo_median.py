...
class WaterQualityProcessorDaily:
    ...
    def process_reservoir(self, reservoir_name, aoi, start_date, end_date, n_tiles=2, output_dir='outputs'):
        """Process a reservoir for each available date in the range"""
        ...
        for date in dates:
            try:
                output_tiff, stats_path = self._process_single_date(
                    reservoir_name, aoi, date, n_tiles, output_dir)
    ...
    def _process_single_date(self, reservoir_name, aoi, date, n_tiles, output_dir):
        ...
        with open(stats_path, 'w') as f:
            ...
            f.write(f"Date: {date}\n")
            f.write(f"Minimum chlorophyll value: {min_max_values.get('chlorophyll_pred_min', 'N/A')}\n")
            f.write(f"Maximum chlorophyll value: {min_max_values.get('chlorophyll_pred_max', 'N/A')}\n")
...
if __name__ == "__main__":
    processor = WaterQualityProcessorDaily(
    ...
    # Process Três Marias
    print("Processing Três Marias reservoir for each available date...")
    results = processor.process_reservoir(
        reservoir_name="tres_marias",
        aoi=aoi_tres_marias,
        start_date='2020-01-01',
        end_date='2020-03-01',
        output_dir='water_analyses/tres_marias_daily'
    )
    
    print("Três Marias processing complete!")
